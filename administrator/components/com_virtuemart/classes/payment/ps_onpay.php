	...
	*/
	function show_configuration() 
	{
		...
		if(htmlspecialchars( $db->sf("payment_extrainfo")) == '' ) 
		{
			...
			echo \'<div style="text-align:left;">\', $db_pm->f("payment_method_name");
			echo \'<form style="text-align:left;" action="\'.$url.\'" method="post" target="_blank">\';
			echo \'<table><tr><td><img src="http://onpay.ru/images/onpay_logo.gif" /></td><td>&nbsp;&nbsp;</td><td><input type="submit" name="submit"  value="\'.$VM_LANG->PHPSHOP_ONPAY_PAY.\'" /></td></tr></table>\';
			echo \'</form></div>\';
  ...
  */
	...
	*/
	...
	function get_onpay_url($order_id,$sum,$user_email) 
	{
		...
		$url="http://secure.onpay.ru/pay/$login?pay_mode=fix&pay_for=$order_id&price=$sum&currency=RUR&convert=yes&md5=$md5check&user_email=$user_email&url_success=$path"; //Формируем платежную ссылку
